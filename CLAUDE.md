...
- **Agents** (`agents/*.md`): frontmatter `name`, `description`, `tools`, `model`, and optionally `effort` (reasoning effort: `low`/`medium`/`high`/`xhigh`/`max`). Two kinds: **review agents** (`code-reviewer`, `security-auditor`, `architect-reviewer`) are report-only — read-only tools (`Read`/`Grep`/`Glob`[/`Bash`]) and a description ending in "Returns actionable findings only." style scoping; **working agents** (`devops-engineer`, `sql-pro`, `pr-creator`) carry `Write`/`Edit` and actually change files (`pr-creator` also commits/pushes).
